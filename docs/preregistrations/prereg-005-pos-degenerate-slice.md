...
**Status: DRAFT-FOR-FREEZE.** Becomes ACTIVE when the pre-freeze items at the end are answered.
The freeze commit hash then goes into `docs/preregistrations/prereg-005.freeze` (same format as
prereg-002/004). Rules may not change between ACTIVE and evaluation (SPEC §6).
...
*complete* certificate of infeasibility. It is not only a companion, unlike at the prereg-002
flat boundary, and unlike prereg-004, where the atoms are irrational.
...
- **U5 formula controls:** `det A = 0` on both slices, so `L` is undefined; `U` is defined and
  equals `a⁴` on D; `μ1 − μ2 = 0` on E, so `U` is undefined. The implementation must not call
  either formula on these slices.
...
| K5 | any verdict reached by calling the undefined L (or, on E, U) formula | implementation REJECTED |
...
## Pre-freeze items
- **P5-OI-1 — slices.** Confirm D (a = 1/2, interior atom) and E (a = 1, atom at the gap edge).
  An a = 0 slice (atom at the origin, H has pivots 1, 0, 0 with μ1 = 0) is possible but adds no
  new code path beyond D/E.
- **P5-OI-2 — zero-pivot direction.** Approve the construction above (normalised so `wᵀSw = −1`)
  as a rev. 2 clarification, or name a different rational normalisation.
- **P5-OI-3 — Farkas role.** Accept the single-atom Farkas certificate as *complete* on
  variance-zero slices (with the Gram dual still required, so each rejection has two independent
  certificates), or keep it as a companion only.
- **P5-OI-4 — resolution.** Keep `1/2^20` for U3.
