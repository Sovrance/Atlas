"""The 4x4 even Weil block ``G[{1, b, b^2, b^3}]`` (ATLAS-RH-ENG-010 §WO-RH-65/67/68).
...
EVEN4_BASIS: Tuple[str, ...] = ("one", "b", "b2", "bcube")

#: A stable identifier for that basis, so a certificate names what it certified.
EVEN4_BASIS_ID = "weil_even4_one_b_b2_bcube_v1"
...
#: Default working precision. The raw fourth minor is ~1e-19 while the entries
...
#: The preconditioner exponents, **frozen for the cell** (§WO-RH-67), in the
#: ENG-008 convention ``D = diag(2^{-e_k})``. The values are
#: ``round(log2(sqrt(G_kk)))`` at the cell midpoint (the first three agree with
#: the ENG-008 even3 choice on the shared elements; the b2 axis rounds to -10
#: here because its diagonal is evaluated at this block's midpoint).
PRECONDITIONER_EXPONENTS: Tuple[int, int, int, int] = (-2, -6, -10, -10)


def basis_identity() -> Dict[str, Any]:
    """What the basis *is*, for a certificate to record rather than imply."""
    return {
        "basis_id": EVEN4_BASIS_ID,
        "elements": list(EVEN4_BASIS),
        "definitions": {
            "one": "1",
            "b": "x(L - x)",
            "b2": "x^2 (L - x)^2 = b(x)^2",
        ...
        "parity_about_midpoint": {n: pole.basis_parity(n) for n in EVEN4_BASIS},
        "spans_in_u": "span{1, u^2, u^4, u^6} with u = x - L/2",
        "kernel_degrees_in_a": {
...
def _as_matrix(entries: Dict[str, Any]) -> List[List[Any]]:
    e = entries
    return [
        [e["G00"], e["G01"], e["G02"], e["G03"]],
        [e["G01"], e["G11"], e["G12"], e["G13"]],
        [e["G02"], e["G12"], e["G22"], e["G23"]],
        [e["G03"], e["G13"], e["G23"], e["G33"]],
    ]
...
def reference_matrix(L: Any) -> List[List[Any]]:
    ...
    """
    return RM.metric_matrix_over(EVEN4_BASIS, L)
...
def matrix_over(lo: float, hi: float, *, precision_bits: int = DEFAULT_PRECISION_BITS,
    ...
    built = assemble_even4_arb(box, precision_bits=precision_bits,
